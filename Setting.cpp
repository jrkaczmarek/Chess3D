...
void Setting::capture(my_model* figure) {
	if (this->colors[figure] == black) {
		for (int j = 0; j < 2; j++) {
			for (int i = 0; i < 8; i++) {
	...
	else if (this->colors[figure] == white) {
		for (int j = 0; j < 2; j++) {
			for (int i = 0; i < 8; i++) {
...
void Setting::initialize_pawns(Player P1, Player P2) {
	for (int i = 0; i < 8; i++) {
...
void Setting::initialize_rooks(Player P1, Player P2) {
...
void Setting::initialize_knights(Player P1, Player P2) {
...
void Setting::initialize_bishops(Player P1, Player P2) {
...
void Setting::initialize_queens(Player P1, Player P2) {
...
void Setting::initialize_kings(Player P1, Player P2) {
...
void Setting::initialize_cylinders(Player P1, Player P2) {
...
void Setting::draw_shift(glm::mat4 M, GLuint texture,
...
void Setting::draw_castling(glm::mat4 M, GLuint texture, float rotation) {
...
void Setting::draw_fall(glm::mat4 M, GLuint texture, GLuint tex_cloth, float rotation) {
	float radius = 0.02;
...
pair <float, float> Setting::calc_direction(chessboard_position X, chessboard_position Y) {
...
float Setting::calc_path_length(pair <float, float> d) {
	return sqrt(pow((d.first), 2) + pow((d.second), 2));
}

void Setting::initialize(Player P1, Player P2, int accuracy) {
...
void Setting::draw_board(glm::mat4 M, GLuint tex_light, GLuint tex_dark) {
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
...
void Setting::draw_captured(glm::mat4 M, GLuint tex_light, GLuint tex_dark) {
	for (int j = 0; j < 2; j++) {
		for (int i = 0; i < 8; i++) {
			if (this->white_captured[i][j] != nullptr) {
...
void Setting::draw_move(glm::mat4 M, GLuint tex_light, GLuint tex_dark, GLuint tex_cloth) {
...
void Setting::next_move(movetype move) {
	this->is_moving = true;
