...
public:
	...
	void draw(glm::mat4 M, GLuint& tex)
	{
		...
		glVertexAttribPointer(spPhong->a("vertex"), 4, GL_FLOAT, false, 0, verts.data());
		glEnableVertexAttribArray(spPhong->a("normal"));
		glVertexAttribPointer(spPhong->a("normal"), 4, GL_FLOAT, false, 0, norms.data());
		glEnableVertexAttribArray(spPhong->a("texCoord"));
		glVertexAttribPointer(spPhong->a("texCoord"), 2, GL_FLOAT, false, 0, texCoords.data());
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, tex);
		glUniform1i(spPhong->u("tex"), 0);
		glDrawElements(GL_TRIANGLES, indices.size(), GL_UNSIGNED_INT, indices.data());
		glDisableVertexAttribArray(spPhong->a("vertex"));
		glDisableVertexAttribArray(spPhong->a("normal"));
		glDisableVertexAttribArray(spPhong->a("texCoord"));
	}
};
